...
- 03=大雨警報、07=波浪警報、14=雷注意報、15=強風注意報、16=波浪注意報、20=濃霧注意報、29=土砂災害警戒情報
- ハマりやすい誤訳: 16 を「大雪注意報」/ 20 を「洪水注意報」とすると間違い。波浪 (うねり) や濃霧の headline と矛盾する。

**status の解釈 (`resolve_warning(code, status)`)**:
...
- 降格 status: `警報から注意報` / `危険警報から注意報` など — これは「以前は上位レベル、今は下位レベル」を意味する **履歴つきの現在状態**
  ...
  - **code 29 (土砂災害警戒情報)** は特殊で、同一 code 内で警戒度が変動する:
    - `発表`/`継続` → 土砂災害警戒情報 / warning
    - `危険警報から注意報` → 土砂災害(注意レベル) / **advisory** (= キキクル注意報相当に緩和済み)
